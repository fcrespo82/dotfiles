...
class Powerline:
    ...
    def appendbefore(self, content, fg, bg, separator=None, separator_fg=None):
    ...
    def draw_segment(self, idx):
        ...
        text = ''.join((
            self.fgcolor(segment[1]), # if segment[0] != '\n' else self.reset,
            self.bgcolor(segment[2]) if segment[0] != '\n' else '',
            segment[0],
            self.bgcolor(next_segment[2]) if next_segment and next_segment[0] != '\n' else self.reset,
            self.fgcolor(next_segment[4]) if next_segment and segment[0] == '\n' else self.fgcolor(segment[4]),
            segment[3]))
        return text
